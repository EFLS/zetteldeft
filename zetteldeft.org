...
Latest additions:
 ...
 - *16 July*: Compile a single =org= that includes tagged files with =zd-org-include-tag=.
...
This string, variable =zd-string-below-title=, can be customized and is empty by default.
...
* Compiling a single =org= from =tags=
** Idea: including notes with given tag

The following functions generate =org= code to include files with a specific =#tag= (or search word in general).
...
#+BEGIN_SRC emacs-lisp :tangle no
(zd-org-include-tagged-files "#export")
#+END_SRC
...
As a solution, use =org-sort= manually after running =zd-org-include-tag=.

** =zd-org-include-tag= generates =#+INCLUDE= code

Asks user for a =#tag= name and inserts headers and =#+INCLUDE= code for all files with said tag.
...
** =zd-org-include-tagged-files= includes files with tag
