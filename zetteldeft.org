...
Latest additions:
 - *14 Feb 2020*: Documentation updates for usage with =spacemacs=.
...
Next, configure =deft= as suggested in section [[#suggested-deft]] and add keybindings as per section [[#suggested-kb]].
...
Installing and enabling =zetteldeft= with the default configuration is straightforward with =spacemacs=:
 ...
 1. Add the =deft= layer to your =.spacemacs= file and enable =zetteldeft= support.
    This will set default keybindings as shared in section [[#kb-spacemacs]].
   #+BEGIN_SRC emacs-lisp :tangle no
   (setq-default dotspacemacs-configuration-layers
     '((deft :variables deft-zetteldeft t)))
   #+END_SRC

 2. Configure =deft= according to section [[#deft-setup]].
...
You'll also want some keybindings. Check out the [[sec:suggested-setup][suggested setup]] below.
...
See [[sec:id-links][below]] for advanced information about IDs and links.
...
How about showing a [[sec:list-tags][list of tags]] or [[sec:gathering-notes][gathering notes]] with a certain search string?
Or maybe a [[sec:visuals][graph]] visualizing how notes are linked?
...
You'll need to set up those yourself, but defaults are suggested at [[sec:suggested-kb][the end of this document]].
...
Keybindings are defined in =deft= layer for =spacemacs=, and are reproduced below for reference. Some keybindings are
globally available and some are only available when in an =org-mode= buffer.
**** Global keys
