...
Latest additions:
 - *11 Mar*: Add =zetteldeft-insert-tag=, thanks to puzan
...
Zetteldeft tags start with a =#= by default, but this can be customized.

Note that this (currently) only affects how tags are /inserted/ via =zetteldeft-insert-tag= -- it doesn't change which tags are /found/.

#+BEGIN_SRC emacs-lisp
(defcustom zetteldeft-tag-prefix "#"
  "String prefix when Zetteldeft tags are inserted."
  :type 'string
  :group 'zetteldeft)
#+END_SRC
...
(defcustom zetteldeft-tag-line-prefix "# Tags "
  "String that indicates which line contains tags."
  :type 'string
  :group 'zetteldeft)
#+END_SRC

**** =zetteldeft-insert-tag= to insert tag from minibuffer

Interactively select a tag to insert, or enter a new one.

Tags are added to the end of the first line that starts with =zetteldeft-tag-line-prefix=.
If no such line is found, the tag is added at point.

#+BEGIN_SRC emacs-lisp
;;;###autoload
(defun zetteldeft-insert-tag ()
  "Select existing tag or enter new one to insert."
  (interactive)
  (zetteldeft--check)
  (let* ((tags (zetteldeft--get-all-sorted-tags))
    ...
    (if dest
        (save-excursion 
          ...
          (insert " " tag))
      (progn
        (message "No tag line found. Is `zetteldeft-tag-line-prefix` configured? Inserting tag %s at point" tag)
        (insert tag)))))
#+END_SRC

For new tags, make sure to prepend the =zetteldeft-tag-prefix= to the new string.
