...
See [[id:0B9AFBF6-6F5D-4607-9FB1-B1DD42BF2C46][below]] for more details on IDs and links.
...
| =zd-avy-file-search=     | Select and follow note link     | =SPC d f=    |
| =zd-avy-link-search=     | Select and seearch for link     | =SPC d l=    |
...
Use avy to jump to a link search for its id.
...
When that is the case, take the directory of the current buffer file name and expand it to its absolute path.
...
One potential *issue*, to be fixed in the future: the file from where the function is called, will also be in the list...
...
The most immediate solution is for the user to be wary of this, and deleting said links and refreshing the deft cache with =deft-refresh= before calling this function.
