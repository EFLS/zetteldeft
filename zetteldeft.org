...
Returns the file title from a file, by default relying on Deft to do so,
thus respecting Deft title rules.

It is possible to provide your own title extraction function as long as it's
compatible with =deft-parse-title=.
