...
Latest additions:
 ...
 - *18 July*: Include a list of links with =zd-insert-list-links=, or =#+INCLUDE= the files with =zd-org-include-files=
...
The following functions automate that process:
 ...
 - =zd-org-include-files= generates =#+INCLUDE= code for =org-mode= for these files.
...
The only function meant for use on the users end, is =zd-org-include-files=.
...
3. *Sorting*
   ...
   As a solution, use =org-sort= manually after running =zd-org-include-files=.

*** =zd-org-include-files= generates =#+INCLUDE= code
...
#+BEGIN_SRC emacs-lisp
(defun zd-org-include-files (zdSrch)
"Inserts at point org-mode syntax to include all files with ZDSRCH. When looking for zetteldeft tags, include the # manually in the search."
