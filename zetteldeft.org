...
**** =zd-id-sanitized= filters zd ID from strings
...
"Strip STRING from everything that is not a number or a dash."
  (with-temp-buffer
    (insert str)
    (when (re-search-forward "[0-9]+\\{4,\\}-[0-9-]+" nil t -1)
      (match-string 0))))
#+END_SRC
...
(zd-id-sanitized "2018-11-09-1934 Schuldenaren in variatie Tst 2000 p. 12-25")
#+END_SRC
...
Tags consist of =#= and at least one or more letters, with =-= allowed.
Tags should be positioned either be at the beginning of a new line, or preceded by a space.
This should help filter out tags used in URLs.
