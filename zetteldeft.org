...
Latest additions:
 ...
 - *4 Nov*: =zd--get-all-tags= and =zd-tag-buffer= find all tags
...
**** =zd-avy-tag-search= selects and searches tags with =avy=
...
**** =zd-avy-link-search= selects and searches links with =avy=

Use avy to jump to a link search for its ID in deft.
This means that each note containing this ID is found.
If you want to open the note with the ID as its name (i.e., follow a link), use =zd-avy-file-search=.
...
**** =zd-avy-file-search= selects and follows links with =avy=
...
*** Utility functions
**** =zd-deft-new-search= starts a new deft search
...
=zd--get-all-tags= puts all tags in =zd--tag-list= and returns them.

Use =zd-tag-buffer= to create a buffer with all tags.

*** =zd--get-all-tags= returns them all
...
The links are temporarily stored in =zd--graph-links=.
