...
| =zd-avy-file-search=     | Select and follow a link        | =SPC d f=    |
| =zd-avy-link-search=     | Select and seearch a link's ID  | =SPC d l=    |
...
To achieve this, get the ID from the current note (which will be nil if there is no zetteldeft ID detected), and remove it from the results as needed.
