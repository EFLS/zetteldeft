...
** NEW =zd-org-include-tag= generates inclusion
...
** FIX =zd-avy-tag-org-include= inserts inclusion of selected tag

One issue: includes the file from which the tag is selected...
...
** =zd-org-include-tagged-files= includes files with search results
