...
* Compiling =org-file= from =tags=
** Idea: compiling & exporting from a tag

The following functions generate =orge= code to include files with a specific =#tag= (or search word in general).
To do so, it inserts a heading, and a line with =#+INCLUDE= and the full path to the relevant notes.

For example,

#+BEGIN_SRC emacs-lisp :tangle no
(zd-org-include-tagged-files "#test")
#+END_SRC

inserts necessary code to include all files containing the tag =#test=.
...
The only function meant for use on the users end, is =zd-org-include-tag=.
...
Solution: filter the file list for specific tags.

See, for example http://ergoemacs.org/emacs/elisp_filter_list.html

** =zd-org-include-tag= generates inclusion

Asks for a =#tag= name and inserst headers and =#+INCLUDE= code for all files with said tag.
Can be used on general search item as well.
When used on =#tags=, make sure to include the =#= manually.
...
** =zd-avy-tag-org-include= inserts inclusion of selected tag

Select a tag through =avy= and include links to all files with the selected tag.

#+BEGIN_SRC emacs-lisp
(defun zd-avy-tag-org-include ()
"Inserts at point org-mode code to include all files with the selected tag."
 (interactive)
 (let (thingPoint)
  (save-excursion
   (avy-goto-char ?#)
   (setq thingPoint (zd-get-thing-at-point)))
  (zd-org-include-tagged-files thingPoint)
))
#+END_SRC

** FIX =zd-org-include-tagged-files= includes files with search results

*Fix* sorting of the list. Currently sorted by with newest on top. Should be reversed?

Inserts =org-mode= code at point to include the notes with a specific search item.
...
Get a list of the files with =#tag=.
List is sorted by title of file, i.e. by link id with newest on top.

#+BEGIN_SRC emacs-lisp
(defun zd-get-file-list (srch)
"Returns a list of files with the search item SRCH."
  ;; Sanitize the srch string
  (setq srch (replace-regexp-in-string "[[:space:]\n]+" " " srch))
  ;; Call deft search on the filter string
...
** =zd-org-include-file= includes a file
