...
*** Finding & linking from minibuffer with full text search

The previous functions allow to search & link based on filename.
Sometimes its more convenient to do these actions based on full text search.
...
This search function can then be used to insert links or open notes.
We can use this to define three new functions: to open a file based on full text search, to insert an ID, or to insert the full title.
...
Meanwhile use keys such as =.=, =<= and =>= to go to the home note, previous buffer or next buffer.
...
Now let's add some custom keys to the default =avy= setup.

For reference, here are some notes on how =avy= works:
- The variable =avy-dispatch-alist= lists actions of what to do when a key not on =avy-keys= is pressed during dispatch.
  The provided action is what is executed once a candidate is finaly pressed.
- The =avy-handler-default= function processes bad read keys.
  Changed with the variable =avy-handler-function=.

So, let's define a separate handler which is used in the browse function above.
Based on the default handler function.
...
By default, =deft-parse-title= is used to do so, meaning Zetteldeft respects Deft title rules the user has configured.
(Note that =deft-use-filename-as-title= will always be =nil=, see below.)

For additional flexibility, the variable =zetteldeft-title-parsing-function= can be customized, but the function this points to should be compatible with =deft-parse-title=, meaning it should take two arguments (full path to the note and contents of the note).
...
The =zetteldeft--lift-file-title= function itself then uses the customized function.
...
A /tag base/ is actually a note that serves as the main note on a specific tag -- collecting the tagged notes by linking to them.

Tags can thus be destinations: a note that explains and searches for the tag.

To find such a note, a (preferably unique) identifier is needed.
This is constructed with the =zetteldeft-tag-base-indicator= (=@= by default, so that the base for =#tag= is =@#tag=).

#+begin_src emacs-lisp
(setq zetteldeft-tag-base-indicator "@")
#+end_src

Each note with =@#tag= will show up, so its good to get search results as a list with =zetteldeft--tag-base-list=.
...
Open tag-base by presenting user with the list.
...
Notes & idea's
- New tag-base: previously for creating new note if none exist
  #+begin_src emacs-lisp :tangle no
            ((eq 0 (length tag-base-results))
            (when (y-or-n-p (format
                            "No tag-base found for \"%s\". Create new?"
                            tag))
              (call-interactively 'zetteldeft-new-file)
              (insert 
                (concat 
                  zetteldeft-tag-base-template-before-tag
                  tag
                  zetteldeft-tag-base-template-after-tag))))
  #+end_src
...
This is an important step in creating structure in your notes.
Zetteldeft can generate a such "list of links".
...
There are two types of list of links:
 - Plain list of notes with a specific search term, listing both ID and note title for each one.
   Such a list can be generated with =zetteldeft-insert-list-links=.
 - Same as the above, but only listing IDs /missing/ from the current note.
   ...
   Generated via =zetteldeft-insert-list-links-missing=.

To make their use more convenient, these functions can be called from an Org-mode source block.
This is the real front-end use of this feature.
...
#+begin_example org
#+BEGIN_SRC emacs-lisp :results silent
(zetteldeft-list-links "#zd-tutorial")
#+END_SRC

 - §2021-04-03-2209 Turn org subheading into a new note
 - §2021-03-29-2226 Tags in Zetteldeft
 - §2020-11-13-2140 Creating useful links
 - §2020-10-24-2304 The Zetteldeft Philosphy
 - §2020-10-06-1014 Exporting with org-publish
 - §2020-09-22-0855 Backlinks
 - §2020-09-20-2106 The home note
#+end_example

Hitting =C-c C-c= in the source block updates the list below the block.
Additional arguments to the =zetteldeft-list-links= function limit the missing links and sorting of the list.
...
**** =zetteldeft-list-prefix= to customize the list markup for list items

To define what a list entry looks like, let's create a customizeable string.
It determines the list markup.
...
*** Semi-automated list of links from an Org source block
**** =zetteldeft-list-links= to be called from block

The following function is meant to be called from an Org source block.
It updates the list below the source block with a new list of links.
...
**** =zetteldeft-insert-list-links-block= inserst code block

Let's also automate the insertion of such a code block.
This function inserts both the block and the list of links.
When called with a prefix, use the "missing"-verion of the list of links.
...
There are two functions to achieve this:
 1. Directly copying the content: =zetteldeft-org-search-insert= inserts the contents of all of these notes below their respective titles.
 2. Using Org-mode =#+INCLUDE= syntax: =zetteldeft-org-search-include= generates =org-mode= syntax to =#+INCLUDE= the files.

**** Including notes with given search term with =zetteldeft-org-search-include=
