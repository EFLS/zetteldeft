...
|--------------------------------+---------------------------------+------------|
| =zetteldeft-new-file=            | Create new note and open        | =SPC d n=    |
| =zetteldeft-new-file-and-link=   | Create new note and insert link | =SPC d N=    |
| =zetteldeft-find-file-id-insert= | Pick a note and insert a link   | =SPC d i=    |
| =zetteldeft-follow-link=         | Follow a link                   | =SPC d f=    |
| =zetteldeft-avy-link-search=     | Select and search a link's ID   | =SPC d l=    |
| =zetteldeft-avy-tag-search=      | Select a tag and search for it  | =SPC d t=    |
| =zetteldeft-search-at-point=     | Search for thing at point       | =SPC d s=    |
| =zetteldeft-search-current-id=   | Search for id of current file   | =SPC d c=    |
...
**** =zetteldeft--insert-title= inserts file title in org-mode
...
When a tag is found, remove any whitespace from it and add it to the =zetteldeft--tag-list= if it isn't there already.
