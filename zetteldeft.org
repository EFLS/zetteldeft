...
As a reminder, again: point =zetteldeft-custom-id-function= to a different function to override the way an ID is generated.
...
Store a link, generate one, and we're done.
...
**** =zetteldeft--current-id= retrieves currentn id
...
**** =zetteldeft--id-to-full-path= returns title from ID
