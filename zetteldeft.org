...
**** =zetteldeft-completing-read= to set a custom completing read function

A function used for some autocompletion.  It must be a function that
is compatible with =completing-read=, for example =ivy-read=.
