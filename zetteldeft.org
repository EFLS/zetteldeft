...
**** =zetteldeft--ensure-deft= ensures deft is loaded

Zetteldeft expects the user to load Deft before using any of its functions.
To assist with this, let's define a function that ensures Deft is loaded -- or, more accurately, that it has hashed the notes.

PROBLEM: Is called too late when function's interactive declaration calls on deft functions?
