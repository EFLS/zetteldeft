...
**** =zetteldeft--ensure-deft= checks whether deft is loaded

Zetteldeft expects Deft to be loaded for many of its functions to work.
Let's include a function that ensures just that -- or, more accurately, that it has hashed the notes.

#+begin_src emacs-lisp
(defun zetteldeft--ensure-deft ()
  "Initialize the Deft cache if it isn't already."
  (unless deft-hash-contents
    (require 'deft)
    (message "Launching Deft...")
