...
Latest additions:
 - *21 Sep 2022*: Add automated list links via Org Dynamic Blocks. See [[#list-links-dblock][documentation below]].
...
Links to those notes are now dead (or rather: broken).
...
*** Semi-atuomated list of links with Org Dynamic Blocks
:PROPERTIES:
:CUSTOM_ID: list-links-dblock
:END:
...
By including a =org-dynamic-block-define=, this dynamic block can now also be inserted via =org-dynamic-block-insert-dblock=.

Next, we need a function that fills the dynamic block with search results.
By using the =org-dblock-write:= format, Org knows how to compute the dynamic block (see the Org manual https://orgmode.org/manual/Dynamic-Blocks.html).

Through the =plist= provided to the Org Dynamic Block we can include functionality to sort the links (with =:sort t=), and only include missing links (with =:missing-only t=).
