...
With prefix, include a `zetteldeft-tag-base-indicator'.
"
  (interactive)
  (zetteldeft--check)
  (let ((dest (when zetteldeft-tag-line-prefix
    ...
    (if dest
        (save-excursion 
          ...
          (end-of-line)
          (insert " ")
          (when current-prefix-arg (insert zetteldeft-tag-base-indicator))
...
(setq zetteldeft-tag-base-indicator "@")

(defun zetteldeft--tag-base-list (tag)
  "Return list of paths to notes that contain tag-base for TAG.

The tag-base is generated by appending tag to zetteldeft-tag-base-indicator"
  (let ((deft-incremental-search nil))  ; Use Deft regexp search mode
    (deft-filter
      (concat
        zetteldeft-tag-base-indicator
        tag)
      t)
    deft-current-files))

;;;###autoload
(defun zetteldeft-tag-base-open (tag)
  "Open the Zetteldeft tag-base for TAG."
  (interactive (list
    (completing-read "Open tag-base: "
      (zetteldeft--get-all-tags))))
  (let ((tag-base-results (zetteldeft--tag-base-list tag)))
    (cond ((eq 1 (length tag-base-results))
           (message "Opening tag base for %s" tag)
           (deft-open-file (car tag-base-results)))
          ((eq 0 (length tag-base-results))
           (message "No note with %s."
                    (concat zetteldeft-tag-base-indicator tag)))
          (t
           (deft-open-file 
            (completing-read
              "Multiple notes with %s:"
              tag-base-results
              ))))))
