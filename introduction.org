...
All link indicators, those =§= symbols, will be replaced by a character that you can type to follow a link of your choice.
...
It asumes basic Emacs knowledge, so I'm going to guess you understand that the code below should go in your =init.el= (or equivalent).
...
Prefer Markdown?
Thats easy enough to change in the example below.
